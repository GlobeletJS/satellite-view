...
`;
  ...
  function initMap(gl) {
    ...
    return init$2({
      ...
      units: "radians",
    }).promise.then(api => setup$1(api, frame.sampler))
      .catch(console.log);
  }

  function setup$1(api, sampler) {
    var loadStatus = 0;
  ...
  function createUniformSetter$1(gl, program, info, textureUnit) {
    const { name, type, size } = info;
    const isArray = name.endsWith("[0]");
    const loc = gl.getUniformLocation(program, name);
  ...
  function createUniformSetters$1(gl, program) {
    ...
    return uniformInfo.reduce((d, info) => {
      ...
      //let setter = createUniformSetter(gl, program, info, textureUnit);
      //d[key] = wrapSetter(setter, isArray, type, size);
      d[key] = createUniformSetter$1(gl, program, info, textureUnit);
  ...
  function initAttributes$1(gl, program) {
    // Construct a dictionary of the indices of each attribute used by program
    const attrIndices = Array
      .from({ length: gl.getProgramParameter(program, gl.ACTIVE_ATTRIBUTES) })
      .map((v, i) => gl.getActiveAttrib(program, i))
  ...
  function initProgram$1(gl, vertexSrc, fragmentSrc) {
    const program = gl.createProgram();
    ...
    const { constantSetters, constructVao } = initAttributes$1(gl, program);
    const uniformSetters = createUniformSetters$1(gl, program);
...
`;
  ...
  function init$3(userParams) {
    ...
    const program = initProgram$1(gl, shaders.vert, shaders.frag);
  ...
  function setup$2(map, gl) {
    ...
    function getCoords() {
      ...
      camPosition[0] = coords["lon"].value / degrees$1;
      camPosition[1] = coords["lat"].value / degrees$1;
