...
`;
  ...
  function setParams$1(userParams) {
    ...
    if (!layers || !layers.length) fail$1$1("no valid array of style layers!");

    let allVectors = layers.every( l => vectorTypes.includes(l.type) );
    if (!allVectors) fail$1$1("not all layers are vector types!");

    let sameSource = layers.every( l => l.source === layers[0].source );
    if (!sameSource) fail$1$1("supplied layers use different sources!");

    if (!source) fail$1$1("parameters.source is required!");

    if (source.type === "vector" && !(source.tiles && source.tiles.length)) {
      fail$1$1("no valid vector tile endpoints!");
    }
...
`;
  ...
  function initMap(context) {
    ...
    return init$2({
      ...
      units: "radians",
    }).promise.then(api => setup$1(api, framebuffer.sampler))
      .catch(console.log);
  }
...
`;
  ...
  function main() {
    ...
    initMap(context)
      .then(map => setup$2(map, context))
      .catch(console.log);
  }

  function setup$2(map, context) {
    var requestID;
    ...
    const view = initView(context.gl.canvas, 25.0);
